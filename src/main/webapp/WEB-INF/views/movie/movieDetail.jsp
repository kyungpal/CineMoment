...
<%--									<c:if test="${oneLineReviewList.oneLineReviewNo == oneLineReviewList.parent_onelinereviewno}">--%>
<%--										asd--%>
<%--									</c:if>--%>
									<th>
<%--										<c:if test="${oneLineReviewList.oneLineReviewNO == }"></c:if>--%>
<%--										<c:out value="${oneLineReviewList.oneLineReviewNO}"></c:out>--%>
<%--										<c:out value="${oneLineReviewList.parent_onelinereviewno}"></c:out>--%>
										<c:choose>
											<c:when test="${oneLineReviewList.oneLineReviewNO eq oneLineReviewList.parent_onelinereviewno}">
											<img src="${contextPath }/resources/image/profile.jpg"width=50px height=50px /></th>
											</c:when>
											<c:otherwise>
												<strong>┗</strong>
											</c:otherwise>
									...
									<td class="review-td ${oneLineReviewList.oneLineReviewNO}"><a style="color: blue;"><strong>${oneLineReviewList.id }</strong></a><br>${oneLineReviewList.content }
									</td>
								</tr>
								<c:if test="${oneLineReviewList.oneLineReviewNO eq oneLineReviewList.parent_onelinereviewno}">
									<tr class="recomment-tr-${oneLineReviewList.oneLineReviewNO}" style="display:none;">
										<th></th>
										<td>
											<form class="recomment-form" action="/movie/oneLineReview.do" method="post">
												<table>
													<tr>
														<th>
															<span>닉네임</span></th>
														<td>
															...
															<input type="hidden" name="parentOneLineReviewNO" value="${oneLineReviewList.oneLineReviewNO}" aria-label="원글번호"/>
															<input type="hidden" name="movie_id" value="${oneLineReviewList.movie_id}" aria-label="영화번호"/>
															<input type="text" name="id" aria-label="닉네임"/>
														</td>
													</tr>
													<tr>
														<th>
														...
														<td>
															<input type="text" name="content" class="recomment-input ${oneLineReviewList.oneLineReviewNO}"/>
															<button type="button" name="recomment-btn" class="recomment-reg-btn ${oneLineReviewList.oneLineReviewNO}">등록</button>
														</td>
													</tr>
												</table>
											</form>
